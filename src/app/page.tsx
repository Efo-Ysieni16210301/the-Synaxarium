...
export default function HomePage() {
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-8">
      ...
      <p className="mb-2 text-sm font-semibold text-ink/60">{t('todaysFeast')}</p>
      ...
      {ethiopianToday && feast && (
        <FeastCard feast={feast} dateLabel={formatEthiopianDate(ethiopianToday, locale)} />
      )}

      {ethiopianToday && !feast && (
        <div className="rounded-2xl border border-gold/40 bg-white/70 p-5 text-ink/70">
          <p className="font-semibold">{formatEthiopianDate(ethiopianToday, locale)}</p>
      ...
      <section className="mt-10">
        <div className="mb-2 flex items-center justify-between">
          <p className="text-sm font-semibold text-ink/60">{t('browseByMonth')}</p>
